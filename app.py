...
# Function to process each frame and predict violence
...
# Streamlit UI
st.set_page_config(layout="wide")  # Use full width of the page
...
if video_file is not None:
    ...
    with col2:
        st.subheader("⚠️ Violence Detection Output")

        # Process video
        best_frame_image, max_confidence = process_video("uploaded_video.mp4")

        if best_frame_image is not None:
            st.image(best_frame_image, caption=f"🚨 Detected Frame (Confidence: {max_confidence*100:.2f}%)", use_container_width=True)
            st.error(f"⚠️ Violence Detected! Confidence: {max_confidence*100:.2f}%")
            
            # Display progress bar for violence confidence
            progress_value = float(max_confidence)  # Explicitly cast to float
            st.progress(progress_value)  # Display the confidence as a progress bar (0 to 1 scale)
